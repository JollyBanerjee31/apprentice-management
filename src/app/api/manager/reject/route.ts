...
export async function POST(request: Request) {
  ...
  const { leaveRequestId, note } = body;
  ...
  const update: Partial<LeaveRequest> = {
    status: "rejected",
    decidedAt: new Date().toISOString(),
  };
  if (typeof note === "string" && note.trim()) {
    update.decisionNote = note;
  }
  await updateLeaveRequest(leaveRequestId, update);
  ...
  if (apprentice) {
    ...
    const balance = apprentice.totalLeave - usedLeave;
    await sendEmail({
      to: apprentice.email,
